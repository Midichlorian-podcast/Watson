...
Tyto konkrétní nálezy byly v tomto průchodu opraveny a automaticky ověřeny. Nelze však poctivě vydat produkční go-live verdikt bez vizuálního browser/axe/E2E průchodu, čerstvého dependency advisory scanu a reálných provider/DR zkoušek. To nejsou skryté vady; jsou evidované jako release blokátory v kapitole 8.
...
| Přístupnost | 0 lint warnings, vlastní kontrakt; browser audit čeká | 7.5/10 |
...
| P0-16 perimeter/supply chain | Opraveno z velké části | CSP, safe errors, rate limits, SHA-pinned CI, Dependabot; audit registry čeká |
| P0-17 320 px a a11y | Staticky opraveno | 0 lint warnings, 92 TSX kontrakt; runtime browser gate čeká |
...
Build nyní selže nad 350 KiB gzip pro největší JS a nad 5.5 MiB offline precache. PWA precachuje jen potřebný encrypted async SQLite WASM. Aktuálně 320 KiB a 4,784 KiB.
...
- Bez čerstvého `pnpm audit --prod` nelze potvrdit absenci známých dependency advisories.
...
Lokální `pnpm audit --prod` nemohl kvůli DNS/sandboxu kontaktovat npm registry a rozšíření oprávnění bylo zamítnuto, protože by odesílalo metadata privátního dependency grafu. CI už audit spouští, ale pro tento snapshot není aktuální výsledek.
...
1. Odemknout browser prostředí a dokončit R-01.
2. Získat CI dependency audit R-02.
