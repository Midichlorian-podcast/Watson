...
Tyto konkrétní nálezy byly v tomto průchodu opraveny a automaticky ověřeny. Chrome runtime/axe/keyboard/zoom důkaz i čerstvý dependency advisory scan už existují. Nelze však poctivě vydat produkční go-live verdikt bez Safari a úplného release E2E průchodu ani bez reálných provider/DR zkoušek. To nejsou skryté vady; jsou evidované jako release blokátory v kapitole 8.
...
| Přístupnost | 0 lint warnings, 105 TSX kontrakt, Chrome axe/keyboard/zoom matrix | 8.5/10 |
...
autentizovaný Chrome CDP fallback funguje. Chrome matrix pokrývá 15 rout na šířkách
320/360/390/768/1024/1440 px ve světlém režimu (90 průchodů) a 390/1440 px v tmavém
režimu (30 průchodů), vždy s emulovaným reduced motion. Axe po opravách vrací nula
critical/serious i nula dalších WCAG A/AA nálezů; matrix nemá horizontální overflow.
Klávesový scénář ověřil otevření dialogu Enterem, deterministický focus trap, Escape
s návratem focusu, otevření mailového vlákna Enterem a ovládání split panelu šipkou.
Domov, Mail a Nastavení prošly 200% page scale při efektivní šířce 720 CSS px bez
overflow a runtime chyby. R-01 ale není celý uzavřen: chybí Safari a kompletní
release E2E scénáře všech níže vyjmenovaných kritických cest.
...
1. Dokončit zbývající Safari a kritické release E2E scénáře R-01; Chrome axe/keyboard/zoom matrix je hotový.
...
- `pnpm audit --prod --audit-level high`: žádná známá zranitelnost v aktuálním lockfile.
...
- Safari část R-01 a kompletní kritické E2E scénáře sign-in, offline/reconnect,
  rejected sync recovery, meeting commit, 2FA a backup/restore v jednom release běhu;
