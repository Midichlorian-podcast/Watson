...
Tyto konkrétní nálezy byly v tomto průchodu opraveny a automaticky ověřeny. Reprodukovatelný Chromium + WebKit runtime/axe/keyboard/reflow důkaz, čerstvý dependency advisory scan i kritický sign-in → offline create → reconnect → edit/move → rejected-sync recovery průchod už existují. Nelze však poctivě vydat produkční go-live verdikt před doplněním browser scénářů meeting commit, 2FA a backup/restore ani bez reálných provider/DR zkoušek. To nejsou skryté vady; jsou evidované jako release blokátory v kapitole 8.
...
| Produkční připravenost | task/offline browser evidence je zelená; chybí zbytek kritických cest, provider a DR | 6.5/10 |
...
Otevřený Add Task i Task Detail prošly WCAG A/AA axe kontrolou; artifact je v
...
globální reduced-motion mezeru pop-in animací. R-01 ale není celý uzavřen: chybí
browser scénáře meeting commit, 2FA a backup/restore. Nativní Safari WebDriver nebyl použit, protože na
...
1. Dokončit zbývající kritické release E2E scénáře R-01; Chromium + WebKit axe/keyboard/reflow a task/offline/recovery průchod jsou hotové.
...
- Reprodukovatelný `pnpm verify:release-e2e` v Chromium i WebKitu ověřil skutečné
  ...
  operace, discard i úspěšný ruční retry s autoritativním receiptem. Add Task a Task
  Detail jsou v tomto průchodu axe-clean; při opravě se doplnil nativní disabled stav
  submitu, přístupný název dialogu/pole a skutečné omezení pop-in animací při
...
- zbývající kritické browser E2E scénáře meeting commit, 2FA a backup/restore;
  sign-in, offline/reconnect a rejected sync recovery už jsou zelené v Chromium i
  WebKitu. Nativní Safari smoke
