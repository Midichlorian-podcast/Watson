...
export function MailSub({
	...
	const draftCount = team.filter((t) => t.draftF || !!m.drafts[t.id]?.text?.trim()).length;
	...
	return (
		<>
			...
			>
				<div
					data-subhead
					style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 2px 5px" }}
				>
					<span
						data-sublbl
						style={{
				...
				<SRow
					active={isF("gatekeeper")}
					onClick={() => m.setFolder("gatekeeper")}
					title="Gatekeeper — noví odesílatelé"
				>
					<svg
						width="15"
						height="15"
				...
				<SRow
					active={isF("f_block")}
					onClick={() => m.setFolder("f_block")}
					title="Blokované a spam"
					pad="6px 10px"
				>
					<svg
						width="15"
