 ...
 * Doručené (Vše / Připnuté / Odloženo / Gatekeeper / Dispečink), Složky,
 * Týmové schránky (barevné tečky, unread per schránka, AI× / warn tečka),
 * Osobní sféra (šifrováno · bez AI). Sekce Správa a Dění přijdou s další várkou.
 */
	...
	return (
		<>
			...
			>
				...
				<SRow active={m.folder === "pinned"} onClick={() => m.setFolder("pinned")} title="Připnuté">
					<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" style={{ flex: "none" }} aria-hidden>
				...
				<SRow active={m.folder === "odlozene"} onClick={() => m.setFolder("odlozene")} title="Odloženo">
					<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" style={{ flex: "none" }} aria-hidden>
				...
				<SRow active={m.folder === "gatekeeper"} onClick={() => m.setFolder("gatekeeper")} title="Gatekeeper — noví odesílatelé">
					<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" style={{ flex: "none" }} aria-hidden>
				...
				<Head>Složky</Head>
				<SRow active={m.folder === "f_sent"} onClick={() => m.setFolder("f_sent")} title="Odeslané" pad="6px 10px">
				...
				<SRow active={m.folder === "f_drafts"} onClick={() => m.setFolder("f_drafts")} title="Koncepty" pad="6px 10px">
					<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinejoin="round" style={{ flex: "none" }} aria-hidden>
				...
				<SRow active={m.folder === "f_arch"} onClick={() => m.setFolder("f_arch")} title="Archiv" pad="6px 10px">
					<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" style={{ flex: "none" }} aria-hidden>
				...
				<SRow active={m.folder === "f_trash"} onClick={() => m.setFolder("f_trash")} title="Koš" pad="6px 10px">
				...
				<SRow active={m.folder === "f_block"} onClick={() => m.setFolder("f_block")} title="Blokované a spam" pad="6px 10px">
					<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" style={{ flex: "none" }} aria-hidden>
						<circle cx="12" cy="12" r="8" />
				...
				{Object.entries(MB).map(([id, mb]) => (
					<SRow key={id} active={m.folder === id} onClick={() => m.setFolder(id)} title={mb.short} pad="6px 10px">
						...
						{mb.warn && !m.adm.fixed && (
							<span
								data-sublbl
								data-health="warn"
								title="Token vyprší za 12 dní — Administrace (další várka)"
