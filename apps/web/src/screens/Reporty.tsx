...
export function Reporty() {
  ...
  const overview = useMemo(() => {
    ...
    return { perDay, weekDone, overdue, avg: (weekDone / 7).toFixed(1).replace(".", ","), maxDay, perProj, maxProj };
  ...
  const memberStats = useMemo(() => {
    ...
    return (uid: string) => {
      ...
      return { mine, open, done, overdue, eff, loadW: Math.min(100, open.length * 13) };
    };
    ...
    <div className="mx-auto max-w-[920px]" style={{ padding: "20px 22px 90px" }}>
      ...
      <div className="mb-3.5 flex items-center gap-2.5">
        ...
        <span
          ...
          style={{ width: 8, height: 8, borderRadius: 3, background: ws?.color ?? "var(--w-brass)" }}
        />
      ...
      {tab === "prehled" ? (
        <>
          ...
          <div className="mb-4 flex gap-3.5">
            <Kpi value={String(overview.weekDone)} label={t("reports.kpiWeek")} color="var(--w-ink)" />
            <Kpi value={String(overview.overdue)} label={t("reports.kpiOverdue")} color="var(--w-overdue)" />
          ...
          {/* cíle */}
          <div className="mt-4 rounded-[14px] border border-line bg-card" style={{ padding: "16px 18px" }}>
            ...
            {reportGoals.map(({ g, pr, st }) => (
              <div key={g.id} className="border-line border-b py-2.5 last:border-b-0">
                <div className="flex items-center gap-2">
                  <span className="min-w-0 flex-1 truncate font-display font-semibold text-ink" style={{ fontSize: 13 }}>
                    {g.name}
                  </span>
                  <span className="shrink-0 font-mono text-ink-3" style={{ fontSize: 11.5 }}>
                    {pr.label}
                  </span>
                  <span className="shrink-0 font-display font-bold text-ink" style={{ fontSize: 12.5 }}>
                    {pr.pct} %
                  </span>
                </div>
                <div className="mt-2 overflow-hidden rounded-full bg-panel-2" style={{ height: 5 }}>
                  <div style={{ height: "100%", width: `${Math.min(100, pr.pct)}%`, background: GSTAT[st][3] }} />
                </div>
              </div>
            ))}
          </div>
            ...
            return (
              ...
              >
                ...
                <div className="shrink-0" style={{ width: 150 }}>
                  <div className="overflow-hidden rounded-[3px] bg-panel-2" style={{ height: 6 }}>
                    <div style={{ height: "100%", width: `${st.loadW}%`, background: "var(--w-brass)" }} />
                  </div>
...
function MemberDetail({
  ...
  goals: { g: GoalRow; pr: ReturnType<typeof goalProgress>; st: keyof typeof GSTAT }[];
  ...
  const roleMut = useMutation({
    ...
    onSuccess: () => void qc.invalidateQueries({ queryKey: ["wsMembersFull", workspaceId] }),
  });

  const byProj = useMemo(() => {
    const m = new Map<string, number>();
    for (const tk of stats.mine) if (tk.project_id) m.set(tk.project_id, (m.get(tk.project_id) ?? 0) + 1);
    return [...m.entries()]
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{ width: 440, maxWidth: "94vw", boxShadow: "var(--w-shadow)", zIndex: 41 }}
      >
        <div className="flex items-center gap-2.5 border-line border-b" style={{ padding: "14px 18px" }}>
          <span className="flex-1 font-display font-bold text-ink-2" style={{ fontSize: 14 }}>
            {t("reports.memberPanel")}
          </span>
          <button type="button" onClick={onClose} aria-label={t("common.cancel")} className="flex text-ink-3 hover:text-ink">
        ...
        <div className="flex-1 overflow-auto" style={{ padding: 18 }}>
          ...
          <div className="overflow-hidden rounded-[4px] bg-panel-2" style={{ height: 8 }}>
            <div style={{ height: "100%", width: `${stats.eff}%`, background: "var(--w-brass)" }} />
          </div>
          ...
          {stats.open.slice(0, 10).map((tk) => {
            ...
            return (
              ...
              >
                ...
                />
                <div className="min-w-0 flex-1">
                  <div className="truncate font-display font-semibold text-ink" style={{ fontSize: 13.5 }}>
                    {tk.name}
                  </div>
                ...
                <span
                  ...
                  style={{ fontSize: 10.5, padding: "2px 7px", color: `var(--w-p${tk.priority ?? 4})` }}
                >
                  P{tk.priority ?? 4}
                </span>
              </button>
          ...
          {byProj.length > 0 && (
            <>
              <SectionLabel style={{ margin: "22px 0 9px" }}>{t("reports.byProjectLabel")}</SectionLabel>
              {byProj.map(({ p, count }) => (
                <div key={p?.id} className="mb-2 flex items-center gap-2">
                  <span
                    ...
                    style={{ width: 8, height: 8, background: p?.color ?? "var(--w-line)" }}
                  />
                  <span className="truncate font-body text-ink-2" style={{ fontSize: 12.5, width: 130 }}>
                    {p?.name}
                  </span>
                  <div className="flex-1 overflow-hidden rounded-[3px] bg-panel-2" style={{ height: 6 }}>
                    <div
                      style={{
                        height: "100%",
          ...
          {goals.length > 0 && (
            <>
              <SectionLabel style={{ margin: "22px 0 9px" }}>{t("reports.goalsLabel")}</SectionLabel>
              {goals.map(({ g, pr, st }) => (
                <div key={g.id} className="border-line border-b py-2.5 last:border-b-0">
                  <div className="flex items-center gap-2">
                    <span className="min-w-0 flex-1 truncate font-display font-semibold text-ink" style={{ fontSize: 13 }}>
        ...
        <div className="border-line border-t" style={{ padding: "13px 18px" }}>
          <button
            type="button"
            onClick={onOpenTasks}
            className="w-full rounded-[10px] font-display font-bold text-white hover:brightness-105"
            style={{ background: "var(--w-brass)", padding: "10px 0", fontSize: 13 }}
          >
            {t("reports.showTasks")}
          </button>
        </div>
...
function Stat({ n, label, color }: { n: number; label: string; color: string }) {
  return (
    <div className="flex-1 rounded-[11px] border border-line bg-panel-2" style={{ padding: "11px 12px" }}>
