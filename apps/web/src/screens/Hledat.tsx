...
/** Česká pluralizace počtu výsledků (1 výsledek / 2–4 výsledky / 5+ výsledků) — verbatim z prototypu. */
function totalLabel(total: number, lang: string): string {
  if (!lang.startsWith("cs")) return `${total} result${total === 1 ? "" : "s"}`;
  return total + (total === 1 ? " výsledek" : total >= 2 && total <= 4 ? " výsledky" : " výsledků");
}
...
export function Hledat() {
  ...
  const { data: steps } = usePsQuery<{ chain_id: string | null; step_state: string | null }>(
  ...
  const res = useMemo(() => {
    ...
    return { tasks: rTasks, projects: rProjects, people: rPeople, flows: rFlows, goals: rGoals, total };
  }, [q, tasks, projects, people, chains, steps, goals, projMap, inboxIds, t, navigate, taskDetail, projectDetail]);
  ...
  return (
    <div className="mx-auto max-w-[760px]" style={{ padding: "20px 22px 90px" }}>
      ...
      >
        <svg width="18" height="18" viewBox="0 0 15 15" fill="none" className="shrink-0 text-ink-3" aria-hidden>
        ...
        {res && (
          <span className="shrink-0 font-mono text-ink-3" style={{ fontSize: 11.5 }}>
            {totalLabel(res.total, i18n.language ?? "cs")}
          </span>
        )}
      </div>
          ...
          {res.tasks.map((r) => (
            <Row key={r.id} onClick={r.run} sub={r.sub}>
              <span
                ...
                style={{ width: 8, height: 8, background: r.color ?? "var(--w-line)" }}
              />
              <span className="min-w-0 flex-1 truncate font-body text-ink" style={{ fontSize: 13.5 }}>
                {r.name}
              </span>
            </Row>
          ))}
        ...
        <Section label={t("search.projects")}>
          {res.projects.map((r) => (
            <Row key={r.id} onClick={r.run} sub={r.sub}>
              <span
                className="shrink-0"
                style={{ width: 10, height: 10, borderRadius: 3, background: r.color ?? "var(--w-line)" }}
              />
        ...
        <Section label={t("search.people")}>
          {res.people.map((r) => (
            <Row key={r.id} onClick={r.run} sub={r.sub}>
              ...
              <span className="min-w-0 flex-1 font-display font-semibold text-ink" style={{ fontSize: 13.5 }}>
                {r.name}
              </span>
            </Row>
          ))}
        ...
        <Section label={t("search.flows")}>
          {res.flows.map((r) => (
            <Row key={r.id} onClick={r.run} sub={r.sub} subMono>
              <svg width="15" height="15" viewBox="0 0 16 16" fill="none" className="shrink-0 text-brass-text" aria-hidden>
                ...
                <path d="M5.3 8 H10.7 M9 6.3 L10.9 8 L9 9.7" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <span className="min-w-0 flex-1 truncate font-display font-semibold text-ink" style={{ fontSize: 13.5 }}>
                {r.name}
              </span>
...
function Row({
  ...
  return (
    ...
    >
      {children}
      <span className={`shrink-0 ${subMono ? "font-mono" : "font-body"} text-ink-3`} style={{ fontSize: subMono ? 11.5 : 12 }}>
        {sub}
      </span>
    </button>
  );
