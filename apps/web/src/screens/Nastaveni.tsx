...
export function Nastaveni() {
	...
	const [landing, setLandingState] = useState<"dnes" | "prehled">(() =>
		storageGet("watson.landing") === "prehled" ? "prehled" : "dnes",
	);
	...
	async function selectRestoreFile(file: File | undefined) {
		...
		try {
			...
			showToast(t("settings.restoreRejected", { code: error instanceof Error ? error.message : "invalid_file" }));
	...
	async function runRestoreDryRun() {
		...
		try {
			...
			showToast(t("settings.restoreRejected", { code: error instanceof Error ? error.message : "restore_failed" }));
		} finally {
	...
	async function runRestoreApply() {
		if (
			...
			restoreReport.checksum !== restoreCandidate.manifest.checksum ||
			restoreBusy
		) return;
	...
	const twoFactorEnabled = Boolean(
		(user as (typeof user & { twoFactorEnabled?: boolean }) | undefined)?.twoFactorEnabled,
	);
	...
	return (
		<div style={{ maxWidth: 680, margin: "0 auto", padding: "20px 22px 90px" }}>
			...
			<div style={{ ...CARD, padding: "16px", marginBottom: 22 }}>
				...
				{!twoFactorEnabled && !twoFactorSetup && (
					<div style={{ marginTop: 14, display: "flex", gap: 8, flexWrap: "wrap" }}>
						...
						<button
							...
							style={{ ...BTN_PRIMARY, opacity: twoFactorBusy ? 0.6 : 1 }}
						>
				...
				{!twoFactorEnabled && twoFactorSetup && (
					<div style={{ marginTop: 16, display: "grid", gap: 14 }}>
						<div>
							...
							<code style={{ display: "block", marginTop: 7, padding: 9, borderRadius: 7, background: "var(--w-panel-2)", overflowWrap: "anywhere", userSelect: "all", fontSize: 11 }}>
						...
						<div>
							...
							<div style={{ marginTop: 7, padding: 10, border: "1px solid var(--w-line)", borderRadius: 8, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 6 }}>
								{twoFactorSetup.backupCodes.map((code) => <code key={code} style={{ userSelect: "all" }}>{code}</code>)}
							</div>
							<label style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 9, fontSize: 12, color: "var(--w-ink-2)" }}>
						...
						<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
							...
							<button
								...
								style={{ ...BTN_PRIMARY, opacity: twoFactorBusy || !twoFactorSaved || !/^\d{6}$/.test(twoFactorCode) ? 0.5 : 1 }}
							>
					...
					<div style={{ marginTop: 14 }}>
						...
						<div style={{ marginTop: 9, display: "flex", gap: 8, flexWrap: "wrap" }}>
							...
							<button
								...
								disabled={twoFactorBusy}
								style={{ ...BTN_GHOST, opacity: twoFactorBusy ? 0.6 : 1 }}
							>
				...
				{twoFactorEnabled && rotatedBackupCodes && (
					<div style={{ marginTop: 14 }}>
						...
						<div style={{ marginTop: 7, padding: 10, border: "1px solid var(--w-line)", borderRadius: 8, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 6 }}>
							{rotatedBackupCodes.map((code) => <code key={code} style={{ userSelect: "all" }}>{code}</code>)}
						</div>
						<label style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 9, fontSize: 12, color: "var(--w-ink-2)" }}>
			...
			<div style={{ ...CARD, marginBottom: 10 }}>
				<div
					style={{ ...ROW, display: "grid", gap: 6, borderBottom: "1px solid var(--w-line)" }}
				>
				...
				>
					...
					<button
						...
						style={{
							...
							cursor:
								backingUp || backupPassphrase.length < 12 ? "default" : "pointer",
						}}
					>
				...
				<div style={{ ...ROW, display: "grid", gap: 12 }}>
					...
					<div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
						...
						<button
							...
							disabled={!restoreCandidate || restoreBusy}
							style={{ ...BTN_GHOST, opacity: !restoreCandidate || restoreBusy ? 0.5 : 1 }}
						>
						...
						<button
							...
							style={{ ...BTN_PRIMARY, opacity: restoreReport?.mode !== "dry-run" || restoreBusy ? 0.5 : 1 }}
						>
							{t("settings.restoreApply")}
						</button>
					</div>
					{restoreFileName && (
						<div style={{ fontSize: 11.5, color: "var(--w-ink-3)" }}>
							{t("settings.restoreSelected", { filename: restoreFileName })}
						</div>
					)}
					{restoreReport && (
						<div role="status" style={{ padding: 10, borderRadius: 8, background: "var(--w-panel-2)", fontSize: 12, color: "var(--w-ink-2)" }}>
							{restoreReport.mode === "dry-run"
								? t("settings.restoreDryRunResult", { insert: restoreReport.totalInserted, skip: restoreReport.totalSkippedExisting })
								: t("settings.restoreApplyResult", { insert: restoreReport.totalInserted, skip: restoreReport.totalSkippedExisting })}
						</div>
					)}
				</div>
			</div>
