...
export function Schranka() {
  ...
  // Cílové projekty pro přeřazení (mimo inbox).
  const targetProjects = useMemo(
    () => projects.filter((p) => !INBOX_NAMES.has(p.name ?? "")),
    [projects],
  );
  ...
  const doUndo = async () => {
    if (!undo) return;
    await powerSync.execute("UPDATE tasks SET due_date = NULL WHERE id = ?", [undo.id]);
    setUndo(null);
  };
    ...
    <div className="mx-auto max-w-[820px]" style={{ padding: "20px 22px 90px" }}>
                ...
                >
                  {tk.project_id && inboxIds.has(tk.project_id) && (
                    <option value={tk.project_id}>
                      {projects.find((p) => p.id === tk.project_id)?.name ?? "Inbox"}
                    </option>
                  )}
      ...
      {undo && (
        <div
          ...
          style={{ transform: "translateX(-50%)", boxShadow: "var(--w-shadow)", zIndex: 40 }}
        >
