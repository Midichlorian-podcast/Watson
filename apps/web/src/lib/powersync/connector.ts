...
export class WatsonConnector implements PowerSyncBackendConnector {
  ...
  async fetchCredentials() {
    const res = await fetch(`${API_URL}/api/powersync/token`, { credentials: "include" });
    ...
    try {
      for (const op of tx.crud) {
        ...
        const res = await fetch(`${API_URL}/api/sync/write`, {
          ...
          credentials: "include",
          body: JSON.stringify({ op: method, table: op.table, id: op.id, data: op.opData }),
        });
        if (!res.ok) {
          const e = new Error(`write HTTP ${res.status}`) as Error & { code?: string };
