...
export function parseRecurrence(text: string, today: string): RecurrenceResult | null {
  ...
  const evenOdd: "even" | "odd" | null = evenSpan ? "even" : oddSpan ? "odd" : null;
  ...
  const inMonthSpan = monthMatch
    ? { start: monthMatch.index!, end: monthMatch.index! + monthMatch[0].length }
    : null;
  ...
  if (wd) {
    ...
    const biSpan = findStem("druh[éouýá]\\p{L}*", s) ?? findStem("dva", s) ?? findStem("dvou", s);
    take(biSpan);
  ...
  if (day != null) {
    ...
    while ((om = re.exec(s))) daySpans.push({ start: om.index, end: om.index + om[0].length });
  } else {
  ...
  const daySp = findStem("den", s) ?? findStem("dny", s) ?? findStem("denn[íěe]", s);
  if (daySp) {
    take(daySp);
  ...
  if (weekSp) {
    ...
    return {
      rule: { kind: bi ? "biweekly" : "weekly", label: bi ? "Každé 2 týdny" : "Každý týden" },
      timeMin,
      consumed,
    };
  }
  ...
  if (monthSp) {
    take(monthSp);
    return { rule: { kind: "monthly", label: "Každý měsíc" }, timeMin, consumed };
  }
  const yearSp = findStem("rok", s) ?? findStem("roce", s) ?? findStem("roky", s);
  if (yearSp) {
    take(yearSp);
