...
export function computeHighlights(
	raw: string,
	hits: { t: string; kind: string }[],
): Highlight[] {
	if (!raw) return [];
	const low = raw.toLowerCase();
	const ranges: Highlight[] = [];
	const taken = (a: number, b: number) =>
		ranges.some((r) => a < r.end && b > r.start);
	const atBoundary = (i: number, len: number) =>
		!isWord(raw[i - 1]) && !isWord(raw[i + len]);
	...
	for (const h of hits) {
		...
		const place = (boundaryOnly: boolean): boolean => {
			...
			while ((idx = low.indexOf(t, from)) !== -1) {
				if (
					!taken(idx, idx + t.length) &&
					(!boundaryOnly || atBoundary(idx, t.length))
				) {
					ranges.push({
						start: idx,
						end: idx + t.length,
