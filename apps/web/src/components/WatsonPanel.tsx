...
export function WatsonPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <>
      ...
      />
      <div
        className="fixed top-0 right-0 bottom-0 flex flex-col border-line border-l bg-panel"
        style={{ width: 384, maxWidth: "92vw", boxShadow: "var(--w-shadow)", zIndex: 43 }}
      >
        {/* header */}
        <div className="flex items-center gap-2.5 border-line border-b" style={{ padding: "16px 18px" }}>
          <span
            className="flex shrink-0 items-center justify-center rounded-full font-display font-bold text-brass-text"
            style={{ width: 30, height: 30, border: "2px solid var(--w-brass)", fontSize: 13 }}
          >
            W
          </span>
        ...
        <div className="flex-1 overflow-auto" style={{ padding: 18 }}>
          <div className="font-display font-bold text-ink" style={{ fontSize: 18, lineHeight: 1.3 }}>
            {greet}
          </div>

          <div
            className="font-display font-bold text-ink-3 uppercase"
            style={{ fontSize: 11, letterSpacing: ".06em", margin: "22px 0 10px" }}
          >
          ...
          {insights.map((i) => (
            <div
              ...
              style={{ borderRadius: 12, padding: "13px 14px", marginBottom: 10 }}
            >
              <div className="flex gap-2.5">
                <span
                  className="shrink-0 rounded-full"
                  style={{ width: 6, height: 6, background: "var(--w-brass)", marginTop: 6 }}
                />
          ...
          <div
            ...
            style={{ marginTop: 18, padding: "14px 16px", background: "var(--w-sidebar)", borderRadius: 12 }}
          >
          ...
          {goals.length > 0 && (
            <>
              <div
                className="font-display font-bold text-ink-3 uppercase"
                style={{ fontSize: 11, letterSpacing: ".06em", margin: "24px 0 8px" }}
              >
