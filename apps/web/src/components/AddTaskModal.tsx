  ...
  return (
    ...
    >
      {dot ? (
        ...
        <span className="shrink-0" style={{ width: 11, height: 11, borderRadius: 3, background: sw }} />
      ) : (
...
export function AddTaskModal({
  ...
  const people = useMemo(
    () => (team ?? []).map((m) => ({ id: m.id, name: m.name, initials: initials(m.name) })),
    [team],
  );
  ...
  // Esc: zavřít popover/našeptávač, pak modal.
  useEffect(() => {
  ...
  function onName(v: string) {
    ...
    const p: Partial<Draft> = { rawName: v, name: parsed.name, highlights: parsed.highlights };
  ...
  const suggest = useMemo(() => {
    ...
    if (mPer) {
      ...
      const list = people
        .filter(
          (p) => p.name.toLowerCase().includes(q) || p.initials.toLowerCase().startsWith(q),
        )
        .slice(0, 5)
        .map((p) => ({
          id: p.id,
  ...
  const addFieldsMore: Field[] = [
    ...
    },
    {
      key: "opakovani",
      icon: "opakovani",
      disp: draft.repeatLabel || (draft.repeat !== "none" ? repLbl[draft.repeat] : t("addmodal.fieldRepeat")),
  ...
  const dateChips: { k: DateKind; l: string; sub: string }[] = [
    ...
    { k: "zitra", l: t("addmodal.dTomorrow"), sub: dmLabel(isoAddDays(today, 1)) },
    { k: "pristi", l: t("addmodal.dNextWeek"), sub: dmLabel(nextMondayISO(today)) },
    { k: "pmonth", l: t("addmodal.dNextMonth"), sub: dmLabel(firstNextMonthISO(today)) },
  ...
  async function submit() {
    ...
    await powerSync.execute(
      `INSERT INTO tasks (id, project_id, name, description, priority, color, due_date, start_date,
        deadline, duration_min, days, recurrence, recurrence_rule, recurrence_basis, assignment_mode, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        draft.project,
    ...
    if (draft.flowAttach) {
      const steps = await powerSync.getAll<{ position: number; step_state: string | null }>(
        ...
        <div className="flex items-start" style={{ gap: 9 }}>
          ...
          <div className="relative flex-1">
            ...
            <textarea
              ...
              style={{ ...titleFont, minHeight: 25, fieldSizing: "content" } as CSSProperties}
            />
          </div>
        </div>
            ...
            {suggest.map((p, i) => (
              <button
                ...
                style={{
                  ...
                  padding: "7px 10px",
                  ...(i === draft.suggestIdx
                    ? { background: "var(--w-card)", boxShadow: "inset 2px 0 0 var(--w-brass)" }
                    : {}),
                }}
              >
                {p.isProj ? (
                ...
                <span className="flex-1 font-display font-semibold text-ink" style={{ fontSize: 13 }}>
                  {p.name}
                </span>
          ...
          >
                    ...
                    .map((p) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className="shrink-0 rounded-full"
                            style={{ width: 9, height: 9, background: p.color ?? "var(--w-ink-3)" }}
                          />
                          <span
            ...
            {draft.pop === "termin" && (
              <>
                ...
                <div className="flex" style={{ gap: 8, marginTop: 9 }}>
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => patch({ dateKind: "custom", customDate: e.target.value })}
                ...
                <div className="flex flex-wrap items-center" style={{ gap: 9, marginTop: 9 }}>
                  ...
                  <div
                    className="flex items-center"
                    style={{ gap: 2, border: "1px solid var(--w-line)", borderRadius: 9, padding: 3 }}
                  >
                    <button
                      type="button"
                      ...
                      style={{ width: 24, height: 24, borderRadius: 7, fontSize: 15 }}
                    >
                      −
                    </button>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        patch({ days: Number.isNaN(n) ? 1 : Math.max(1, Math.min(60, n)) });
                      }}
              ...
              <>
                ...
                {nAssign >= 2 && (
                  <div className="flex" style={{ gap: 6, marginTop: 9 }}>
                    <ChipBtn on={draft.assignMode === "any"} onClick={() => patch({ assignMode: "any" })}>
                      {t("addmodal.modeAny")}
                    </ChipBtn>
                    <ChipBtn on={draft.assignMode === "all"} onClick={() => patch({ assignMode: "all" })}>
                      {t("addmodal.modeAll")}
                    </ChipBtn>
                  </div>
                )}
            ...
            {draft.pop === "trvani" && (
              <div className="flex flex-wrap items-center" style={{ gap: 6 }}>
                ...
                <label
                  className="flex items-center"
                  style={{ gap: 5, border: "1px solid var(--w-line)", borderRadius: 9, padding: "5px 9px" }}
                >
                  <input
                    type="number"
                    min={0}
                    ...
                    onChange={(e) => {
                      const n = Number.parseInt(e.target.value, 10);
                      patch({ duration: Number.isNaN(n) ? 0 : Math.max(0, Math.min(10080, n)) });
                    }}
            ...
            {draft.pop === "deadline" && (
              <>
                ...
                >
                  <svg width="14" height="14" viewBox="0 0 15 15" fill="none" aria-hidden className="shrink-0">
                    <path
                ...
                {deadlineBad && (
                  <div
                    ...
                    style={{ gap: 6, marginTop: 6, fontSize: 11.5, color: "var(--w-overdue)" }}
                  >
                ...
                {richActive && draft.repeatLabel && (
                  ...
                  >
                    <span
                      ...
                      style={{ fontSize: 10, letterSpacing: ".05em", color: "var(--w-brass-text)" }}
                    >
                      {t("addmodal.fromText")}
                    </span>
                    ...
                    <button
                      type="button"
                      onClick={() => patch({ repeat: "none", repeatRule: null, repeatLabel: "" })}
                  ...
                  >
                    <div>
                      <div
                        ...
                        style={{ fontSize: 10, letterSpacing: ".05em", marginBottom: 6 }}
                      >
                      ...
                      <div className="flex flex-wrap items-center" style={{ gap: 6 }}>
                        ...
                        {draft.repeatEndKind === "until" && (
                          <input
                            ...
                            style={{ borderRadius: 8, padding: "5px 8px", fontSize: 12.5 }}
                          />
                        )}
                        {draft.repeatEndKind === "count" && (
                          <span className="inline-flex items-center" style={{ gap: 6 }}>
                            <input
                              ...
                              style={{ width: 58, borderRadius: 8, padding: "5px 8px", fontSize: 12.5 }}
                            />
                ...
                {draft.attached.map((name, i) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        patch({ attached: draft.attached.filter((_, j) => j !== i) })
                      }
        ...
        >
          {needsName ? (
            <span
              ...
              style={{ gap: 6, fontSize: 11.5, color: "var(--w-overdue)", lineHeight: 1.5 }}
            >
              <svg width="13" height="13" viewBox="0 0 14 14" fill="none" aria-hidden className="shrink-0">
                <path d="M7 1.5 L13 12 H1 Z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" />
